...
def load_weights_sequential(target, source_state):
    model_to_load= {k: v for k, v in source_state.items() if k in target.state_dict().keys() and k != 'conv1.weight'}
    
    state = target.state_dict()
    state.update(model_to_load)
...
class ResNet(nn.Module):
    def __init__(self, block, layers=(3, 4, 23, 3), inchannel = 3):
        self.inplanes = 64
...
class DenseNet(nn.Module):
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, pretrained=True, inchannel = 3):

        super(DenseNet, self).__init__()

        # First convolution
        self.start_features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(inchannel, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
...
class SqueezeNet(nn.Module):
    ...
    def __init__(self, pretrained=False, inchannel = 3):
        super(SqueezeNet, self).__init__()

        self.feat_1 = nn.Sequential(
            nn.Conv2d(inchannel, 64, kernel_size=3, stride=2, padding=1),
...
def squeezenet(pretrained=True, inchannel = 3):
    return SqueezeNet(pretrained, inchannel = inchannel)


def densenet(pretrained=True, inchannel = 3):
    return DenseNet(pretrained=pretrained, inchannel = inchannel)


def resnet18(pretrained=True, inchannel = 3):
    model = ResNet(BasicBlock, [2, 2, 2, 2], inchannel = inchannel)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34(pretrained=True, inchannel = 3):
    model = ResNet(BasicBlock, [3, 4, 6, 3], inchannel = inchannel)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50(pretrained=True, inchannel = 3):
    model = ResNet(Bottleneck, [3, 4, 6, 3], inchannel = inchannel)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet50']))
    return model


def resnet101(pretrained=True, inchannel = 3):
    model = ResNet(Bottleneck, [3, 4, 23, 3], inchannel = inchannel)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet101']))
    return model


def resnet152(pretrained=True, inchannel = 3):
    model = ResNet(Bottleneck, [3, 8, 36, 3], inchannel = inchannel)
    if pretrained:
